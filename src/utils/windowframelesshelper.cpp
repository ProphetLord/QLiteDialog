...
void WindowFramelessHelper::pressEvent(QMouseEvent *event) {
    if(moveWidget == nullptr) {
        return;
    }
    // 鼠标左键按下事件
    if (event->button() == Qt::LeftButton && !moveWidget->window()->isMaximized() && (moveWidget->rect().contains(event->pos()) ||
...
MouseRegion WindowFramelessHelper::getMouseRegion(QPoint mousePos) {
    ...
    if (x >= w - 4 && y > offset && y < h - offset) {
        return MouseRegion::Window_Right_Border;
    }
    if (x <= 4 && y > offset && y < h - offset) {
...
void WindowFramelessHelper::stretchWindowByMouse(QMouseEvent *event) const {
    if(!windowResizeable) {
        return;
    }
